...
.controller('HomeCtrl', ['$scope', function ($scope) {
    ...
    $scope.menuItems = [
      ...
      { icon: 'far fa-fw fa-envelope', name: 'Get Email Updates', url: '/updates' },
...
// Email Updates
//////////////////////////////////////////////////////////
angular.module('whiteflag.updates',[])

.config(['$routeProvider',function($routeProvider) {
  $routeProvider.when('/updates', { templateUrl: 'updates.html', controller: 'UpdatesCtrl' });
}])

.controller('UpdatesCtrl', ['$scope','$http','DataService', function($scope, $http, DataService) {
  $scope.topBanner = 'updates.jpg';
  $scope.blanks = [undefined,null,''];
  $scope.updatesError = false;
  $scope.updatesData = {
    subject: 'White Flag Mobile: Email Updates Subscribe',
    to: 'shawn@whiteflagcalvary.org',
    from: 'donotreply@whiteflagcalvary.org',
    html: ''
  };

  $scope.prepUpdatesRequest = function(){
    $scope.updatesError = false;
    $scope.updatesHtml = '';
    $scope.updatesHtml += "<div>Please add the following person to the White Flag Updates email list:</div><br /><br />";
    if($scope.blanks.indexOf($scope.updatesName)==-1){
      $scope.updatesHtml += '<div><b>Name:</b>&nbsp;&nbsp;' + $scope.updatesName + '</div>';
    }
    if($scope.blanks.indexOf($scope.updatesEmail)==-1){
      $scope.updatesHtml += '<div><b>Email:</b>&nbsp;&nbsp;' + $scope.updatesEmail; + '</div>';
    }
    if($scope.blanks.indexOf($scope.updatesEmail)==-1){
      $scope.sendUpdatesRequest();
    }else{
      $scope.updatesError = true;
    }
  }

  $scope.sendUpdatesRequest = function(){
    $scope.updatesData.html = $scope.updatesHtml;
    var updatesData = $scope.updatesData;
    DataService.sendMail(updatesData)
    .then(function(resp){
      console.log(resp);
      if(resp=='OK'){
        $scope.updatesSentMsg = 'Your email updates request has been sent.';
      }else{
        $scope.updatesSentMsg = 'Sorry, your email updates request was not sent.';
      }
  ...
  $scope.resetUpdatesForm = function(){
    $scope.updatesHtml = '';
    $scope.updatesName = null;
    $scope.updatesEmail = null;
